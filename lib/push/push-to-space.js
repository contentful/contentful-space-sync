...
export default function (responses, managementClient, spaceId) {
  ...
  const deletedContentTypes = filter(destination.contentTypes, contentType => {
    return !find(source.contentTypes, 'original.sys.id', contentType.sys.id)
  })

  return managementClient.getSpace(spaceId)
  .then(space => {
    return publishing.makeEntitiesUnpublisher(space, deletedContentTypes, 'ContentType')()
    .delay(PRE_PUBLISH_DELAY)
    .then(deletion.makeEntitiesDeleter(space, deletedContentTypes, 'ContentType'))
    .then(publishing.makeEntitiesUnpublisher(space, source.deletedEntries, 'Entry'))
    .delay(PRE_PUBLISH_DELAY)
