 ...
 */
export function processAssets (space, assets) {
 ...
 */
export function checkAssets (space, pendingAssets, allAssets = pendingAssets) {
  return Promise.map(pendingAssets, pendingAsset => {
    return space.getAsset(pendingAsset.sys.id)
  ...
  .then(pendingAssets => {
    ...
    return pendingAssets.length > 0
      ? checkAssets(space, pendingAssets, allAssets)
      : allAssets
  })
}
...
function checkIfAssetHasProcessed (allAssets, pendingAsset, processedAsset) {
  const stillProcessing = every(processedAsset.fields.file, (file) => !file.url)
  if (!stillProcessing) {
