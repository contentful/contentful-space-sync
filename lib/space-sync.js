...
const nextSyncTokenFile = tokenDir + `/contentful-space-sync-token-${opts.sourceSpace}-to-${opts.destinationSpace}`
...
function dumpAndWarnAboutErrorBuffer (message = 'Additional errors were found') {
  ...
  if (loggedErrors.length > 0) {
    const errorLogFileName = `${tokenDir}/contentful-space-sync-${Date.now()}.log`
