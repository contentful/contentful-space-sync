...
function createEntitiesInSpace (space, entities, destinationEntities, type) {
  return Promise.map(entities, entity => {
    // TODO
    // find entity.sys.id in destinationEntities
    // if it exists, get the version, and replace the method name below
    // repeat this for entries method
    debugger
    return space[`create${type}`](entity.transformed)
    .then(entityCreationSuccess, err => {
      if (err.name === 'VersionMismatch') {}
      // if version mismatch somehow skip this and stuff
      throw err
    })
  })
}
...
function makeEntriesCreator (space, entries) {
  return function createEntriesInSpace () {
    return Promise.map(entries, entry => {
      return space.createEntry(entry.original.sys.contentType, entry.transformed)
      .then(entityCreationSuccess, err => {
